//+------------------------------------------------------------------+
//|          Position Sync EA v10 Nuxt - CLOSED ONLY                 |
//|                  Nuxt API Version                                |
//+------------------------------------------------------------------+
#property strict

// ========== NUXT API CONFIGURATION ==========
// Update base URL sesuai dengan domain Nuxt Anda
input string   API_BASE_URL  = "https://your-domain.com";  // Base URL Nuxt application
input string   API_URL       = "/api/history";  // API endpoint path (akan digabung dengan BASE_URL)
...
int OnInit()
{
   ...
   {
      Alert("Gagal sync awal v10 Nuxt! Cek log.");
      return INIT_FAILED;
   }
   if (ENABLE_LOGS) Print("=== EA v10 Nuxt CLOSED ONLY Started ===");
   if (ENABLE_LOGS) Print("Nuxt API URL: ", fullApiUrl);
...
{
   EventKillTimer();
   if (ENABLE_LOGS) Print("=== EA v10 Nuxt CLOSED ONLY Stopped ===");
}
...
bool SyncPositions(bool isInitialSync)
{
   ...
   if(ENABLE_LOGS) 
   {
      Print("Syncing: ", closed_count, " closed positions ONLY to Nuxt API");
   }
   
   if (SendToNuxtAPI(jsonData))
   {
      lastSyncTime = toTime;
      if (ENABLE_LOGS) Print("✓ Sync SUCCESS to Nuxt API at ", TimeToString(toTime));
      return true;
   }
   
   if (ENABLE_LOGS) Print("✗ Sync FAILED to Nuxt API");
   return false;
}
...
bool SendToNuxtAPI(string jsonData)
{
   char post[], result[];
   
   // Build URL dengan token sebagai query parameter (Nuxt API format)
   string url_with_token = fullApiUrl;
   if(StringFind(fullApiUrl, "?") == -1)
   {
   ...
   // Headers untuk Nuxt API (JSON content type)
   string headers = "Content-Type: application/json\r\n";
   headers += "Token: " + API_TOKEN + "\r\n";
   headers += "User-Agent: MT5-EA-v10-Nuxt\r\n";
   ...
   if(res == -1)
   {
      ...
      if(ENABLE_LOGS) 
      {
         Print("✗ WebRequest FAILED to Nuxt API. Error: ", error);
         if(error == 4014) 
         {
   ...
   if(ENABLE_LOGS) Print("Nuxt API Response: ", response);
   
   // Check response status (Nuxt API returns JSON with status field)
   if(StringFind(response, "\"status\"") >= 0 && StringFind(response, "\"success\"") >= 0)
   {
      return true;
   }
   
   return true; // Return true as long as request succeeded
}
